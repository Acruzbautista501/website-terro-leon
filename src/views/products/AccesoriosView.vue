...
function irADetalle(accesorio: { titulo: string; tipo: string }) {
  const slug = slugify(accesorio.titulo);
  router.push({ name: 'AccesorioDetalle', params: { tipo: accesorio.tipo, slug } });
}
</script>
  ...
  <main class="bg-color">
    <section class="container-fluid">
      <div class="p-5 text-center container">
        <div class="text-center py-4">
          <h1 class="title-hero-product mb-4 display-1 text-uppercase">Accesorios</h1>
        </div>
      </div>
    ...
    <section class="container py-5">
      <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-5">
        <div
          v-for="accesorio in accesoriosConTipo"
          :key="`${accesorio.tipo}-${accesorio.titulo}`"
          class="col"
        >
          <div
            class="card border-0 shadow-sm h-100 text-center teja-card"
            @click="() => { irADetalle(accesorio); handleScrollToTop(); }"
          >
            <div class="ratio ratio-1x1">
              <img
                :src="accesorio.img?.[0] || '/img/placeholder.jpg'"
                class="card-img-top"
                :alt="'Imagen de ' + accesorio.titulo"
              />
            </div>
            <div class="card-body">
              <h5 class="card-title text-uppercase fs-3">{{ accesorio.titulo }}</h5>
            </div>
          </div>
        </div>
      </div>
